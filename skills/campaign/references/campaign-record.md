...
| `project` | optional; a repository-root-relative or absolute path | The repository the campaign's **work** lives in, when that is not the repository the manifest lives in. `--root` names where the manifest and the harness are; `project` names where the roadmap and the branches are. Two things depend on it and both fail the same silent way when it is missing: the roadmap is read from `<root>/.claude/docs/roadmap/roadmap.json` — which a project does not have — and landedness is computed in a repository without the branches, so every row reads `absent`, and absent is indistinguishable from a real negative. Omit it for a campaign whose work is in the same repository; a declared path that does not resolve is `undetermined`, never a fall back. |
