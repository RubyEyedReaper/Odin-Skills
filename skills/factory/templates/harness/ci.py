...
def resolve(argv: list[str]) -> list[str]:
    ...
    if found is None and head in ("python", "python3"):
        # ODIN FORK. Same failure this function exists to prevent, one platform over.
        # Debian, Ubuntu and their derivatives ship `python3` and NO `python` — so the
        # scaffold's own default commands (`python -m compileall`, `python -m unittest`)
        # fail there with "No such file or directory: 'python'", and `ci.py` reports it as
        # `GATE_FAILED: static`. That reads as "your code does not compile" and it is
        # nothing of the kind. Measured: 36 of the 56 runner tests failed on this alone.
        # `sys.executable` is the interpreter already running the harness, so it exists by
        # construction and is the one whose stdlib the checks were written against.
