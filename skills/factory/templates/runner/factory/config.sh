...
# THE FILE HALF, DEFINED ONCE (ADR-0170). Two callers ask this question -- the dispatcher, and
# any entry point that refuses before spending -- and each used to carry its own copy. The copies
# had already drifted: `[ -f ]` in the dispatcher against `[ -e ]` in the lap workflow, so a
# DIRECTORY named .factory/STOP halted one of them and not the other, and the workflow restated
# this file's default path inline where a change here would have left it stale and silently in
# charge of its own caller.
#
# What is shared is the QUESTION. Each caller keeps its own answer: the dispatcher exits 0,
# because a stop is not a failure and cron reads the exit code, while a workflow node exits
# non-zero, because that is the only way it halts a DAG. That difference is a disposition, not
# drift.
#
# The remote half is `factory/state.py stop-requested` and stays there. This half must work with
# the network down -- and with python missing -- which is the whole reason there are two.
factory_stopped_by_file() {     # factory_stopped_by_file  ->  0 when the stop FILE is present
  [ -f "$FACTORY_STOP_FILE" ]
}

# The first line of the stop file, or NOTHING when there is no reason to give. Callers print it;
# nothing branches on it, so an empty answer is silence rather than a placeholder somebody has to
# recognise.
factory_stop_reason() {         # factory_stop_reason  ->  the reason, or empty
  [ -s "$FACTORY_STOP_FILE" ] && head -1 "$FACTORY_STOP_FILE"
}
...
# --- FACTORY_PYTHON (ODIN FORK) -----------------------------------------------------------
#
# The interpreter every script in this directory calls. It used to be the literal word
# `python`, in six scripts and sixty-odd places, and on Debian, Ubuntu and their derivatives
# there is no such binary — those ship `python3` and nothing else. The failure is not a clean
# one: `state.py` and `guard.py` exit 127, the shell prints "python: command not found", and
# the surrounding script reports the result as `GATE_FAILED`, `ILLEGAL_TRANSITION`, or a
# dispatcher that answers `idle` forever. Measured on this fork: 36 of 56 runner tests failed,
# every one of them naming a defect that was not there.
#
# It belongs here rather than in each script, for exactly the reason the header of this file
# gives: if you are editing another script to change a value, that is a bug in this file.
FACTORY_PYTHON="${FACTORY_PYTHON:-$(command -v python3 || command -v python || echo python3)}"
