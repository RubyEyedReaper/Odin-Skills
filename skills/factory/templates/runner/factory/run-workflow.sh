...
trap '"$FACTORY_PYTHON" factory/cost.py record "$RUN" >/dev/null 2>&1 || true' EXIT

log() { echo "[$(date -u +%FT%TZ)] [$WORKFLOW/$TARGET] $*"; }

state()   { "$FACTORY_PYTHON" factory/state.py "$@"; }
...
read_fm() { "$FACTORY_PYTHON" factory/state.py get "$2" 2>/dev/null | grep "^$1=" | head -1 | cut -d= -f2- || true; }
note()    { "$FACTORY_PYTHON" factory/state.py comment "$1"; }   # body on stdin

escalate() {            # escalate <reason>
  log "ESCALATE: $*"
  "$FACTORY_PYTHON" factory/state.py set "$TARGET" state=needs-human || true
  ...
  ESC_ISSUE="$("$FACTORY_PYTHON" factory/state.py get "$TARGET" 2>/dev/null | grep '^issue=' | head -1 | cut -d= -f2- || true)"
  if [ -n "${ESC_ISSUE:-}" ] && [ "$ESC_ISSUE" != "$TARGET" ]; then
    "$FACTORY_PYTHON" factory/state.py set "$ESC_ISSUE" state=needs-human >/dev/null 2>&1 || true
  ...
  if [ "$GH" -eq 1 ]; then
    ...
    "$FACTORY_PYTHON" factory/state.py escalate-note "$TARGET" "$WORKFLOW" "$*" || true
  fi
  exit 3
}
...
materialise() {         # materialise <issue-target>
  ...
  "$FACTORY_PYTHON" factory/state.py body "$1" > "$ISSUE_FILE" \
...
node() {                # node <name> <model> <prompt-file> <allowed-tools>
  ...
  if ! "$AGENT" -p "$(cat "$rendered")" \
    ...
    "$FACTORY_PYTHON" factory/node_failure.py "$RUNDIR/nodes/$name.json" "$name" "$MAX_BUDGET" \
  ...
  # asked twice to run a `"$FACTORY_PYTHON" -c` its allowlist did not cover, was refused, replied "the
  ...
  "$FACTORY_PYTHON" factory/node_failure.py --denials-only "$RUNDIR/nodes/$name.json" "$name" \
...
case "$WORKFLOW" in
  ...
  implement-issue)
    ...
    "$FACTORY_PYTHON" factory/state.py set "$TARGET" state=in-progress || escalate "illegal state transition"
    ...
    (
      ...
      # `Bash("$FACTORY_PYTHON" -c:*)` added 2026-08-10. The node could previously execute exactly one
      ...
      node implement "$MODEL_CHEAP"   "$ROOT/factory/prompts/implement.md" \
           "Read,Glob,Grep,Edit,Write,Bash("$FACTORY_PYTHON" -c:*),Bash($FACTORY_VALIDATE_QUICK)"
    ) || { git worktree remove "$WT" --force >/dev/null 2>&1 || true
    ...
    if [ -s "$RUNDIR/ASSUMPTIONS" ]; then
      ...
      # `sed FILE | head -20` strands sed on SIGPIPE, and under `set -euo pipefail` a bare
      # pipeline's 141 ends the workflow node mid-report (harness:RM-0506). Bound the producer.
      head -20 "$RUNDIR/ASSUMPTIONS" | sed 's/^/    /'
    fi
    ...
    "$FACTORY_PYTHON" factory/guard.py --base "$BASE" --head "$BRANCH" \
      || { git worktree remove "$WT" --force >/dev/null 2>&1 || true
    ...
    if [ "$GH" -eq 1 ]; then
      ...
      "$FACTORY_PYTHON" -c "import io,sys;t=io.open(sys.argv[1],encoding='utf-8').read();print(t.split('---',2)[2].lstrip() if t.startswith('---') else t)"         "$PRFILE" > "$RUNDIR/pr.body.md"
      ...
      PR_OUT="$("$FACTORY_PYTHON" factory/state.py create-pr "$PR_TITLE" main "$BRANCH" \
                  < "$RUNDIR/pr.body.md")" \
      ...
      "$FACTORY_PYTHON" factory/state.py set "gh:pr:$PR_NUM" state=open \
  ...
  validate-pr)
    ...
    "$FACTORY_PYTHON" factory/tripwire.py "$WT" \
      || { git worktree remove "$WT" --force >/dev/null 2>&1 || true
    ...
    "$FACTORY_PYTHON" factory/state.py set "$TARGET" state=validating \
      || { git worktree remove "$WT" --force >/dev/null 2>&1 || true
    ...
    "$FACTORY_PYTHON" factory/guard.py --base "$BASE" --head "$CHECKOUT" > "$RUNDIR/gate.log" 2>&1 \
      || { git worktree remove "$WT" --force >/dev/null 2>&1 || true
           ...
           "$FACTORY_PYTHON" factory/state.py set "$TARGET" state=rejected || true
  ...
  fix-pr)
    ...
    git worktree remove "$WT" --force >/dev/null 2>&1 || true
    "$FACTORY_PYTHON" factory/state.py bump-attempt "$TARGET" \
    ...
    "$FACTORY_PYTHON" factory/state.py set "$TARGET" state=open \
  ...
  triage)
    ...
    "$FACTORY_PYTHON" - "$DEC" <<'PY' > "$RUNDIR/triage.env" || escalate "triage decision is not readable JSON"
...
PY
    ...
    # header, piped a `"$FACTORY_PYTHON" -c` through the platform codepage, and handed the result to
    ...
    "$FACTORY_PYTHON" factory/state.py triage-note "$TARGET" "$DEC"       || escalate "the decision was made but could not be published; a verdict the filer cannot read is not a verdict"

    if [ -n "$DEC_PRIORITY" ]; then
      "$FACTORY_PYTHON" factory/state.py set "$TARGET" "priority=$DEC_PRIORITY" || true
    fi
    "$FACTORY_PYTHON" factory/state.py set "$TARGET" "state=$DEC_STATE" \
    ...
    if [ "$DEC_STATE" = "needs-human" ]; then
      TRIAGE_WHY="$("$FACTORY_PYTHON" -c "import json,sys;print((json.load(open(sys.argv[1],encoding='utf-8')).get('note') or 'escalated at triage').strip().replace(chr(10),' ')[:300])" "$DEC" 2>/dev/null || echo "escalated at triage")"
