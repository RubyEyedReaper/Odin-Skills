...
status_linux_macos() {
  ...
  # Captured: the timer listing is unbounded and `| grep -q` under pipefail reports an ARMED timer
  # as absent once grep exits before systemctl has finished writing (harness:RM-0506).
  if command -v systemctl >/dev/null 2>&1; then
    timers="$(systemctl list-timers 2>/dev/null)"
  else
    timers=""
  fi
  if grep -q "$TASK_NAME" <<< "$timers"; then
