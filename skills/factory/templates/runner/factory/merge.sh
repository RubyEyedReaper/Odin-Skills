...
# `… | grep | head -1 | cut` leaves grep writing into a closed pipe once head exits, and under
# `set -euo pipefail` that 141 becomes this function's status and ends the merge (harness:RM-0506).
# `grep -m1` bounds the producing stage instead, so nothing exits early on a live stream.
read_fm() {
  local _fm
  _fm="$("$FACTORY_PYTHON" factory/state.py get "$PR_FILE")"
  grep -m1 "^$1=" <<< "$_fm" | cut -d= -f2-
}
...
"$FACTORY_PYTHON" factory/guard.py --base "$BASE" --head "$HEAD_REF" \
...
git merge-base --is-ancestor "$BASE" "$HEAD_REF" \
  || {
       ...
       if "$FACTORY_PYTHON" factory/state.py set "$PR_FILE" state=open >/dev/null 2>&1; then
...
if [ "$GH" -eq 1 ]; then
  ...
  PR_STATE="$("$FACTORY_PYTHON" -c "import json,sys;print(json.loads(sys.argv[1])['state'])" "$PR_JSON")"
  PR_MERGEABLE="$("$FACTORY_PYTHON" -c "import json,sys;print(json.loads(sys.argv[1])['mergeable'])" "$PR_JSON")"
  PR_DRAFT="$("$FACTORY_PYTHON" -c "import json,sys;print(json.loads(sys.argv[1])['isDraft'])" "$PR_JSON")"
  PR_BASE="$("$FACTORY_PYTHON" -c "import json,sys;print(json.loads(sys.argv[1])['baseRefName'])" "$PR_JSON")"
  ...
  PR_MERGE_STATE="$("$FACTORY_PYTHON" -c "import json,sys;print(json.loads(sys.argv[1]).get('mergeStateStatus',''))" "$PR_JSON")"
...
"$FACTORY_PYTHON" factory/state.py set "$PR_FILE" state=merged \
  || BOOKKEEPING_FAILED="$BOOKKEEPING_FAILED PR-record($PR_FILE)"

if [ "$GH" -eq 1 ]; then
  "$FACTORY_PYTHON" factory/state.py set "$ISSUE" state=done \
    || BOOKKEEPING_FAILED="$BOOKKEEPING_FAILED issue($ISSUE)"
else
  "$FACTORY_PYTHON" factory/state.py set "issues/$(basename "$ISSUE")" state=done 2>/dev/null \
    || "$FACTORY_PYTHON" factory/state.py set "$ISSUE" state=done \
    || BOOKKEEPING_FAILED="$BOOKKEEPING_FAILED issue($ISSUE)"
fi
