...
note() { "$FACTORY_PYTHON" factory/state.py comment "$PR_FILE"; }

fail() {
  echo "GATE_FAIL: $*" >&2
  "$FACTORY_PYTHON" factory/state.py set "$PR_FILE" state=needs-human >/dev/null 2>&1 || true
  ...
  ISSUE_REF="$("$FACTORY_PYTHON" factory/state.py get "$PR_FILE" 2>/dev/null | grep '^issue=' | head -1 | cut -d= -f2- || true)"
  if [ -n "${ISSUE_REF:-}" ]; then
    "$FACTORY_PYTHON" factory/state.py set "$ISSUE_REF" state=needs-human >/dev/null 2>&1 || true
...
PR_STATE="$("$FACTORY_PYTHON" factory/state.py get "$PR_FILE" | grep '^state=' | cut -d= -f2)"
...
if [ -n "${FAILED_STEP:-}" ]; then
  # Captured, then cut to length in the shell. `grep … | head -1 | head -c 300` leaves grep writing
  # into a closed pipe, and under `set -euo pipefail` the substitution aborts the very failure
  # report it is composing (harness:RM-0506).
  CONTEXT_LINE="$(grep -m1 -B1 "GATE_FAILED: $FAILED_STEP" "$LOG" | awk 'NR==1')"
  fail "the suite stopped at the '$FAILED_STEP' step, so every check after it never ran. ${CONTEXT_LINE:0:300}"
fi
...
if [ -f "$FACTORY_E2E_FLOOR_FILE" ]; then
  FLOOR=$("$FACTORY_PYTHON" -c "import json,sys;print(json.load(open(sys.argv[1])).get(sys.argv[2],0))" \
...
if [ -z "$ASSUMPTIONS" ]; then
  ISSUE_FOR_ASSUMPTIONS="$("$FACTORY_PYTHON" factory/state.py get "$PR_FILE" 2>/dev/null | grep '^issue=' | head -1 | cut -d= -f2- || true)"
...
if [ -n "$ASSUMPTIONS" ]; then
  AUTOMERGE_ALLOWED=0
  echo "GATE_NOTE: this change rests on $(grep -c . <<< "$ASSUMPTIONS") recorded assumption(s); auto-merge refused, a human decides."
fi

# --- 4. the verdict ----------------------------------------------------------
[ -s "$VERDICT" ] || fail "verdict file is empty or missing - the judge step produced nothing, which is not an approval"
"$FACTORY_PYTHON" -c "import json,sys;json.load(open(sys.argv[1]))['verdict']" "$VERDICT" \
  || fail "verdict file is not parseable JSON with a .verdict field"

DECISION=$("$FACTORY_PYTHON" -c "import json,sys;print(json.load(open(sys.argv[1]))['verdict'])" "$VERDICT")
SUMMARY=$("$FACTORY_PYTHON" -c "import json,sys;print(json.load(open(sys.argv[1])).get('summary','no summary'))" "$VERDICT")

case "$DECISION" in
  approve)
    if [ "$AUTOMERGE_ALLOWED" -eq 0 ]; then
      "$FACTORY_PYTHON" factory/state.py set "$PR_FILE" state=passed \
...
' "$ASSUMPTIONS"
  ...
  request_changes)
    ...
    "$FACTORY_PYTHON" factory/state.py set "$PR_FILE" state=failed \
    ...
    ;;
  reject)
    "$FACTORY_PYTHON" factory/state.py set "$PR_FILE" state=rejected \
