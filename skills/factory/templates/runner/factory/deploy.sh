...
if [ "${1:-}" = "--rollback" ]; then
  ...
  # `tail -2 | head -1` makes `head` exit while `tail` is still writing, which under
  # `set -euo pipefail` aborts the rollback (harness:RM-0506). `awk NR==1` reads to EOF.
  PREV=$(tail -2 "$HISTORY" | awk 'NR==1 {print $2}')
