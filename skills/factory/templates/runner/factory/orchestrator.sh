...
# The same shape, for the same reason, for the stop button's file half (ADR-0170). config.sh owns
# the predicate now; a config.sh predating that decision sets FACTORY_STOP_FILE and defines no
# function, and the STOP BUTTON is the last thing that may fail because the file you were told to
# edit is an older copy. Falls back to exactly the inline test this script used to carry.
command -v factory_stopped_by_file >/dev/null 2>&1 || factory_stopped_by_file() {
  [ -f "$FACTORY_STOP_FILE" ]
}
command -v factory_stop_reason >/dev/null 2>&1 || factory_stop_reason() {
...
if factory_stopped_by_file; then
  ...
  STOP_WHY="$(factory_stop_reason)"
  [ -n "$STOP_WHY" ] && log "reason: $STOP_WHY"
  exit 0
fi
...
STOP_OUT="$("$FACTORY_PYTHON" factory/state.py stop-requested 2>&1)" || {
...
$("$FACTORY_PYTHON" factory/state.py list prs --state validating 2>/dev/null || true)
EOF
...
$("$FACTORY_PYTHON" factory/state.py next 2>/dev/null | grep '^stalled' | cut -f2 || true)
EOF
fi
...
if [ "$AUTONOMY" -lt 1 ]; then
  ...
  log "would run: $("$FACTORY_PYTHON" factory/state.py next | tr '\t' ' ')"
  exit 0
fi
...
while [ "$SLOTS" -gt 0 ]; do
  ...
  escalate)
    ...
    [ "$DRY_RUN" -eq 0 ] && {
      ...
      CAP_ISSUE="$("$FACTORY_PYTHON" factory/state.py get "$TARGET" 2>/dev/null | grep '^issue=' | head -1 | cut -d= -f2- || true)"
      if [ -n "${CAP_ISSUE:-}" ]; then
