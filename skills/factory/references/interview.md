...
> **ODIN FORK — THE ROUNDS RESOLVE, THEY DO NOT WAIT.**
>
> Upstream runs this interview through `AskUserQuestion`. Odin never calls it, in any phase, for
> any reason (ADR-0052, `.claude/rules/common/decision-authority.md`): a question is not a pause,
> it is the end of the turn, and nothing behind it resumes until a human comes back. Three rounds
> of questions would stop an unattended build three times before a line is written.
>
> **Everything below still happens except the waiting.** Compute the question. Draft the options
> from their PRD and their repo, and cite the source. Mark exactly one `(Recommended)`, first, with
> the reason in its one-line description. Then **adopt that option and record it** — as a numbered
> DEC from `decision-matrix` where the options are scorable, or in the plan doc's decision-forks
> section where they are not. Say in one line what was chosen and why, and that it is reversible.
>
> The record is what the gate was for. An approval survives no context reset; a decision recorded
> with its criteria can be re-run with different weights by whoever reads it next.
>
> **Two things this does not license.** It does not license inventing an answer: an option drafted
> from nothing is a guess, and the rule below about deriving options from their own material is the
> whole reason the discharge is safe. And it does not license silence — every adopted answer is
> reported, in a line, so the user can overturn it when they read the result.
>
> Attended sessions may still ask, and the upstream text below is then correct as written.
...
**EVERY QUESTION IS ASKED IN THE SAME SHAPE — and then answered by you** (Odin fork; see the
overlay at the head of this file). Not most questions, not the ones with obvious options: every
one, including the open-ended ones, gets drafted options, one recommendation, and a recorded
answer. The shape is the hard requirement. The waiting is what Odin removes.
...
>
> **Odin fork:** the same reasoning survives the discharge and is why it is safe. A recorded
> decision IS the closest-of-three, already corrected once — against the PRD and the repo rather
> than against the user's memory — and the user overturns it by reading one reported line instead
> of by answering three rounds of prompts. What would NOT survive is inventing options where the
> material gives none: there, say so, take the conservative reading, and record the uncertainty as
> the decision's own expiry condition.
...
**Odin fork — the playback is a WRITE, not a message.** Upstream ends here in the conversation.
Here it ends in `.claude/docs/plans/<date>-<factory>.md`: the spec below becomes that plan doc's
decision-forks section plus its recorded scope decision, which is what CLAUDE.md item 5 requires
before any file is written anyway. Report two lines to the human — the plan's path, and the
autonomy level being built for. Nothing about this interview survives a context reset unless it is
on disk.
