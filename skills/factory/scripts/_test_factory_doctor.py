...
# GIT_DIR overrides where `git init` (no explicit path) creates its repository — a
# hook or `git rebase -x` exporting it would redirect this fixture's own init
# (harness:RM-0317/RM-0318).
_GIT_ENV_VARS = (
...
def build(root: pathlib.Path) -> None:
    ...
    subprocess.run(["git", "init", "-q"], cwd=root, capture_output=True, env=_scrubbed_env())
