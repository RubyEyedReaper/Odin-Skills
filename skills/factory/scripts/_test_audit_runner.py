...
# ODIN FORK. This was an absolute path to the skill author's own workstation
# (`C:\Users\...`), so the suite raised FileNotFoundError on every other machine
# in existence — a test suite that has never run anywhere but one laptop. Resolved
# from this file instead, which is the only anchor that travels with the fork.
