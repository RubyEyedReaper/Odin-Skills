...
# GIT_DIR (a hook, `git rebase -x`) overrides where a directory-less `git init`
# creates its repository, and overrides `-C`'s discovery entirely for every other
# call (harness:RM-0317/RM-0318) — scrub it from every git subprocess this file starts.
_GIT_ENV_VARS = (
...
def _scrubbed_env(base=None):
    base = os.environ if base is None else base
    return {k: v for k, v in base.items() if k not in _GIT_ENV_VARS}

HERE = Path(__file__).resolve().parent
SKILL = HERE.parent

# ODIN FORK. The suite drove the fixtures with the literal word `python`, which does not
# exist on Debian, Ubuntu or their derivatives — so 36 of the 56 tests failed there, each
# reporting the defect it locks rather than the missing binary. `sys.executable` is the
# interpreter already running the suite, so it exists by construction.
PY = sys.executable
...
class Factory:
    ...
    def py(self, args: str, env: dict | None = None):
        return self.sh(f"{PY} factory/state.py {args}", env=env)
...
def build_pristine(dest: Path) -> Path:
    ...
    # ODIN FORK. `-b main`, because the fixture must establish every input its verdict turns
    # on. Without it the branch name comes from the HOST's `init.defaultBranch`, and a host
    # that has never set it gets `master` — while the runner's own default is `BASE=main`. The
    # result is `fatal: invalid reference: main` swallowed by `worktree add ... 2>&1`, which
    # surfaces as `ESCALATE: could not create worktree`, and two tests then report the defects
    # they lock rather than the ambient git config that actually broke them.
    subprocess.run(["git", "init", "-q", "-b", "main"], cwd=dest, check=True, env=_scrubbed_env())
    for k, v in (("core.autocrlf", "false"), ("user.email", "f@f.f"),
                 ("user.name", "fixture"), ("commit.gpgsign", "false")):
        subprocess.run(["git", "config", k, v], cwd=dest, check=True, env=_scrubbed_env())
...
""")
    ...
    cfg = re.sub(r'FACTORY_VALIDATE_CMD="\$\{FACTORY_VALIDATE_CMD:-[^}]*\}"',
                 f'FACTORY_VALIDATE_CMD="${{FACTORY_VALIDATE_CMD:-{PY} harness/ci.py}}"', cfg)
    cfg = re.sub(r'FACTORY_VALIDATE_QUICK="\$\{FACTORY_VALIDATE_QUICK:-[^}]*\}"',
                 f'FACTORY_VALIDATE_QUICK="${{FACTORY_VALIDATE_QUICK:-{PY} harness/ci.py --quick}}"', cfg)
    ...
    f.issue("0001-widget")
    subprocess.run(["git", "add", "-A"], cwd=dest, check=True, env=_scrubbed_env())
    subprocess.run(["git", "commit", "-q", "-m", "fixture"], cwd=dest, check=True, env=_scrubbed_env())
    return dest
...
def test_guard_blocks_a_branch_touching_factory(f: Factory):
    ...
    rc, out = f.sh(f"{PY} factory/guard.py --base main --head factory/evil")
...
def test_guard_sees_inside_new_directories(f: Factory):
    ...
    rc, out = f.sh(f"{PY} factory/guard.py --base main --head factory/sneaky")
...
def test_doctor_detects_an_unedited_scaffold(f: Factory):
    rc, out = f.sh(f'{PY} "{(SKILL / "scripts" / "factory_doctor.py").as_posix()}" '
...
def install_harness(f: Factory, e2e: str = E2E_TRIVIAL, unit_tests: int = 9,
    ...
    cfg["driver"] = "library"
    cfg["library"] = {"import_check": f'{PY} -c "import app"'}
    cfg["static"] = f"{PY} harness/_static.py"
    cfg["unit"] = f"{PY} harness/_unit.py"
...
def test_harness_names_the_failing_rung(f: Factory):
    install_harness(f, static=f"{PY} harness/_boom.py")
...
MUTATIONS = [
    ...
    Mutation(
        ...
        '"$FACTORY_PYTHON" factory/state.py set "$CAP_ISSUE" state=needs-human >/dev/null 2>&1 || true',
    ...
    Mutation(
        ...
        'NEXT="$("$FACTORY_PYTHON" factory/state.py next --exclude "$EXCLUDE")"',
        'NEXT="$("$FACTORY_PYTHON" factory/state.py next)"',
