...
// Where a relative path in the tool call resolves against, and what
// `isInsideProject()` measures against. This must stay `event.cwd`-first: a
// shell-inferred write (`cp a.css styles.css`, a heredoc, a Python write_text)
// carries a *relative* destination that belongs to the frame the harness
// reported, not to the repo root. Do not merge this with resolveStateCwd()
// below — ADR-0042 records why they are separate.
...
// Where the hook's own state lives — `.impeccable/config.json`, `hook.cache.json`,
// `hook.pending.json`. This is the opposite precedence, and deliberately so: the
// harness-supplied `event.cwd` follows the session, which drifts the moment any
// prior command runs `cd projects/<slug> && …`, and a drifted state path scatters
// untracked cache directories through the tree (RM-0016; ADR-0032 fixed the same
// class one frame further out). `envProjectDir(null)` returns falsy when no
// harness env var is set, so non-Claude harnesses keep the old behaviour.
export function resolveStateCwd(event, fallback = process.cwd()) {
  return envProjectDir(null)
    || event?.cwd
    || (Array.isArray(event?.workspace_roots) && event.workspace_roots[0])
    || fallback;
}
...
// Anchor to the true project root via a harness-provided env var rather than
// trusting process.cwd(), which reflects whatever directory a prior shell
// command left the session in (e.g. `cd projects/<slug> && npm test`) — not
// necessarily the repo root. CLAUDE_PROJECT_DIR is Claude Code's own
// guaranteed-set path env var (see .claude/rules/web/hooks.md); check it
// first since this harness's primary target is Claude Code, not Cursor.
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    ...
    // Two roots, two jobs (ADR-0042). `projectCwd` resolves what was edited and
    // must follow the event; `stateCwd` locates the hook's own files and must
    // follow the anchored project root instead.
    const projectCwd = event.cwd || cwd;
    const stateCwd = resolveStateCwd(event, cwd);
    ...
    const config = readConfig(stateCwd);
    if (config.enabled === false) {
      return result({ skipped: 'config-disabled', durationMs: Date.now() - started });
    }

    const cache = readCache(stateCwd);
    const sessionId = event.session_id || 'unknown';
    const det = detector || await loadDetector();
    if (!det || typeof det.detectText !== 'function') {
      persistCache(stateCwd, cache);
      return result({ skipped: 'detector-missing', durationMs: Date.now() - started });
    }
    // The design system is project state (`.impeccable/design.json`, `DESIGN.md`),
    // so it resolves against the same root as the config that governs it.
    const scanOptions = designSystemOptions(config, det, stateCwd);
    ...
    for (const filePath of targetFiles) {
      ...
      // Relative to the root the config came from — `ignoreFiles` globs are
      // written against the project the config sits in, not against whichever
      // subtree the session drifted into.
      const relForMatch = relativize(filePath, stateCwd);
