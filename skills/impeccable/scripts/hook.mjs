...
import { runHook, writeAuditLog, envProjectDir } from './hook-lib.mjs';
...
async function main() {
  ...
  // Anchor to the true project root, not whatever directory a prior shell
  // command left the session in — see envProjectDir() for why.
  const projectDir = envProjectDir(process.cwd());
  ...
  writeAuditLog(process.env, result.audit, projectDir);
