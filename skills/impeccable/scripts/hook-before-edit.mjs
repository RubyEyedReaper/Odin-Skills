...
async function main() {
  ...
  // `cwd` resolves the proposed target (relative paths in shell-inferred writes
  // belong to the reported frame); `stateCwd` locates the hook's own config and
  // cache, which must not follow the session's drift — ADR-0042.
  const cwd = resolveProjectCwd(event);
  const stateCwd = resolveStateCwd(event);
  ...
  const rel = relativePath(filePath, stateCwd);
  ...
  const scanOptions = designSystemOptions(config, detector, stateCwd);
