...
#          [--crop x,y,w,h] [--bg auto|none|#rrggbb] [--key flood|global] [--tolerance N] [--scale N] [--smooth R] [--colors N]
#          [--color original|currentColor] [--allow-background] [--set key=value]...
#          [--iou N] [--mae N] [--edge-f1 N]
#
...
[ $# -ge 1 ] || usage
src="$1"; shift
name="" kind="" out="" crop="" bg="auto" key="flood" tolerance="40" scale="1" smooth="0" colors="0" color="original" allow_bg=""
trace_sets=() qa_args=()
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --iou|--mae|--edge-f1) need "$@"; qa_args+=("$1" "$2"); shift 2 ;;
...
prep_args=("$src" "$prepared" --trace-input "$work/trace.png" --colors "$colors" --smooth "$smooth" --bg "$bg" --key "$key" --tolerance "$tolerance" --scale "$scale")
[ -n "$crop" ] && prep_args+=(--crop "$crop")
[ "$color" = currentColor ] && prep_args+=(--mono)
i2c_py prep "${prep_args[@]}" || fail prep $? 2

stage trace "$kind preset"
binary=() mono=()
[ "$color" = currentColor ] && { binary=(--binary); mono=(--mono); }
