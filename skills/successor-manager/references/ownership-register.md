...
| `model` | the tier the session was launched on | A measured $0.00 Sonnet share across 4,915 successor sessions was an absent flag, not a preference, and absent flags fail silently and forever (ADR-0097, on `origin/harness/successor-burn-audit`). A launch tier nobody records is one nobody notices reverting. |
